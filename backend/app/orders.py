...
def load_dataset() -> OrderDataset:
    return OrderDataset.model_validate_json(
        (DATA_DIR / "orders.json").read_text(encoding="utf-8-sig")
    )


def load_products() -> dict[str, dict]:
    rows = json.loads((DATA_DIR / "products.json").read_text(encoding="utf-8-sig"))["rows"]
    if not isinstance(rows, list) or any(
        not isinstance(row, dict)
        or not isinstance(row.get("SKU"), str)
        or not isinstance(row.get("ProductName"), str)
...
def price_of(product: dict | None) -> Decimal | None:
    try:
        ...
        if value is None or not value.is_finite() or not 0 <= value <= 100000000:
            return None
        # RRP is an AUD amount in cents; never silently turn fractional cents into a new price.
        return value if value == value.quantize(CENT) else None
...
def shipping_estimate(lines: list[dict], postcode: str, carrier: str, enabled: bool) -> dict:
    ...
    for line in lines:
        ...
        unit_volume = measurement(
            p.get("volume", ""),
            {"mm³": Decimal("0.001"), "cm³": Decimal("1"), "m³": Decimal("1000000")},
        )
        if unit_volume is None:
            dimensions = [
                measurement(
                    p.get(key, ""), {"mm": Decimal("0.1"), "cm": Decimal("1"), "m": Decimal("100")}
                )
...
def build_orders(
    dataset: OrderDataset, products: dict[str, dict], estimate: bool = False
) -> list[dict]:
    output = []
    for order in dataset.orders:
        ...
        for item in (line for line in dataset.line_items if line.order_no == order.order_no):
            ...
            lines.append(
                {
                    **item.model_dump(),
                    "name": product.get("ProductName", item.sku).strip() if product else item.sku,
        ...
        for shipment in (s for s in dataset.shipments if s.order_no == order.order_no):
            ...
            shipments.append(
                {
                    **shipment.model_dump(),
                    "skus": [line["sku"] for line in shipment_lines],
                    "shipping": shipping_estimate(
                        shipment_lines, order.postcode, shipment.carrier, estimate
                    ),
                }
            )
        ...
        output.append(
            {
                **order.model_dump(mode="json"),
                ...
                "totals": {
                    ...
                    "subtotal": money(subtotal) if not warnings else None,
                    "gst": money(gst) if not warnings else None,
                    "shipment_fee": money(shipment_fee),
                    "rounding_adjustment": money(rounded_subtotal - displayed_lines)
                    if not warnings
                    else None,
